"""
Botanika QR Code Generator - Print Quality
Generates QR codes for all 5 branches in 2 styles:
- Style 1: Green background (#1B5E3A) with white QR dots
- Style 2: White background with green frame and green QR dots

Output: 1000x1000 PNG + SVG for each branch
"""
...
# Frame widths
GREEN_BG_FRAME_WIDTH = 20  # Thin green frame for style 1
WHITE_BG_FRAME_WIDTH = 20  # Green frame thickness for style 2

# Paths
LOGO_PATH = r"G:\My Drive\Simon\Botanika\Logo\leaf only.jpg"
OUTPUT_DIR_GREEN = r"C:\Users\LENOVO.LENOVO\AI\botanika\frontend\public\images\qr\print\green-bg"
OUTPUT_DIR_WHITE = r"C:\Users\LENOVO.LENOVO\AI\botanika\frontend\public\images\qr\print\white-bg"
...
def generate_green_bg_qr(url, output_png, output_svg, logo):
    """
    Style 1: Green background with white QR dots
    - Rounded rectangle overall shape
    - Green background #1B5E3A
    - White QR modules
    - Thin green frame border
    - Logo with gold frame in center
    - Transparent corners
    """
    print(f"  [Green BG] Generating PNG...")

    qr = generate_qr_matrix(url)

    # Create QR with white dots on green background
    qr_img = qr.make_image(fill_color=WHITE_COLOR, back_color=GREEN_COLOR)
    qr_img = qr_img.convert('RGBA')

    # Calculate sizes
    inner_size = OUTPUT_SIZE - (GREEN_BG_FRAME_WIDTH * 2)
    ...
    # Paste QR code
    canvas.paste(qr_img, (GREEN_BG_FRAME_WIDTH, GREEN_BG_FRAME_WIDTH))
    ...
    if logo is not None:
        ...
        if framed_logo:
            ...
            # Clear area behind logo with green
            clear_pad = 8
            clear = Image.new('RGBA', (fw + clear_pad*2, fh + clear_pad*2), GREEN_COLOR + (255,))
    ...
    # Generate SVG
    print(f"  [Green BG] Generating SVG...")
    generate_svg_green_bg(url, output_svg)

def generate_white_bg_qr(url, output_png, output_svg, logo):
    """
    Style 2: White background with green QR dots
    - Rounded rectangle overall shape
    - FULL WHITE background (same as green version has full green background)
    - Green QR modules
    - Logo with gold frame in center
    - Transparent corners outside rounded rectangle
    - NO GREEN BORDER - this is the inverse of green version
    """
    ...
    inner_size = OUTPUT_SIZE - (WHITE_BG_FRAME_WIDTH * 2)
    qr_img = qr_img.resize((inner_size, inner_size), Image.LANCZOS)
    ...
    # Draw FULL WHITE background (rounded rectangle) - NO GREEN BORDER
    draw.rounded_rectangle(
        [0, 0, OUTPUT_SIZE-1, OUTPUT_SIZE-1],
        radius=CORNER_RADIUS,
    ...
    canvas.paste(qr_img, (WHITE_BG_FRAME_WIDTH, WHITE_BG_FRAME_WIDTH))
...
def generate_svg_green_bg(url, output_svg):
    """Generate SVG with green background and white QR modules"""
    qr = generate_qr_matrix(url)
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  <rect x="0" y="0" width="{svg_size}" height="{svg_size}" rx="45" ry="45" fill="#1B5E3A"/>
...
def main():
    print("=" * 60)
    print("Botanika QR Code Generator - Print Quality")
    print("=" * 60)
    print(f"Output size: {OUTPUT_SIZE}x{OUTPUT_SIZE}px")
    print(f"Green background style: {OUTPUT_DIR_GREEN}")
    print(f"White background style: {OUTPUT_DIR_WHITE}")
    print()

    # Ensure directories exist
    os.makedirs(OUTPUT_DIR_GREEN, exist_ok=True)
    ...
    for branch_name, filename, url in BRANCHES:
        ...
        png_green = os.path.join(OUTPUT_DIR_GREEN, f"{filename}.png")
        svg_green = os.path.join(OUTPUT_DIR_GREEN, f"{filename}.svg")
        generate_green_bg_qr(url, png_green, svg_green, logo)

        # White background style
        png_white = os.path.join(OUTPUT_DIR_WHITE, f"{filename}.png")
        svg_white = os.path.join(OUTPUT_DIR_WHITE, f"{filename}.svg")
        generate_white_bg_qr(url, png_white, svg_white, logo)

        print()

    print("=" * 60)
    print("[Progress] COMPLETE - All QR codes generated!")
    print("=" * 60)
    ...
    for branch_name, filename, url in BRANCHES:
        files = [
            os.path.join(OUTPUT_DIR_GREEN, f"{filename}.png"),
            os.path.join(OUTPUT_DIR_GREEN, f"{filename}.svg"),
            os.path.join(OUTPUT_DIR_WHITE, f"{filename}.png"),
            os.path.join(OUTPUT_DIR_WHITE, f"{filename}.svg"),
        ]
        for f in files:
    ...
    print()
    if all_files_exist:
        print("[SUCCESS] All 20 files generated successfully!")
    else:
